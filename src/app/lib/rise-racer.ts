...
const getRiseRacerContract = async (
  ...
  if (signerOrProvider) {
    ...
    if (typeof (signerOrProvider as ethers.Signer).getAddress === "function") {
    ...
    console.warn(
      "No signer/provider passed and window.ethereum not found. Using default provider."
    );
 ...
 */
export const getVelocityPerClick = async (
 ...
 */
export const getCurrentVelocity = async (
  playerAddress: string,
  provider?: ethers.Provider
): Promise<bigint> => {
  // Pass only the provider if available, otherwise let getRiseRacerContract handle default/signer logic
...
export interface PlayerInfoFull {
  velocity: bigint; // Current velocity
  clickPower: bigint;
  riseCrystalBalance: bigint;
  currentStage: number; // Current stage
  currentUniverse: bigint; // Current universe ID
  totalClicks: bigint; // Total number of clicks
  isStaking: boolean; // Whether player is staking
}
...
export const getPlayerInfo = async (
  ...
  return {
    ...
    clickPower: playerInfo[1],
    riseCrystalBalance: playerInfo[2],
    currentStage: playerInfo[3],
    currentUniverse: playerInfo[4],
    totalClicks: playerInfo[5],
    isStaking: playerInfo[6],
  };
};
...
export const clickRace = async (
  ...
  if (!signer.provider) {
    throw new Error(
      "Signer must be connected to a Provider to send a transaction."
    );
  }
  ...
  const receipt = await sendRawTransactionSync(
    signer.provider as ethers.JsonRpcProvider,
    signedTxData
  );
