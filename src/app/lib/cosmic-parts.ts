...
const cosmicPartAbi = [
  ...
  },
  {
    ...
    inputs: [
      ...
      { name: "", type: "uint8", internalType: "enum ICosmicParts.PartType" },
    ],
    outputs: [{ name: "", type: "uint256", internalType: "uint256" }],
    stateMutability: "view",
  },
    ...
    outputs: [
      {
        name: "",
        type: "tuple",
        internalType: "struct ICosmicParts.CosmicPart",
        components: [
          {
            name: "partType",
            type: "uint8",
            internalType: "enum ICosmicParts.PartType",
          },
          { name: "level", type: "uint256", internalType: "uint256" },
          { name: "boost", type: "uint256", internalType: "uint256" },
        ],
      },
    ],
    stateMutability: "view",
...
const getCosmicPartsContract = async (
 ...
 */
export const getTotalBoost = async (
  address: string,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 */
export const getPartData = async (
  tokenId: bigint,
  provider?: ethers.Provider
): Promise<CosmicPart> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 */
export const getChassisCost = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 */
export const getChassisVelocity = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the upgrade cost (as bigint)
 */
export const getEngineCost = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the velocity boost (as bigint)
 */
export const getEngineVelocity = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the upgrade cost (as bigint)
 */
export const getWheelCost = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the velocity boost (as bigint)
 */
export const getWheelVelocity = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the upgrade cost (as bigint)
 */
export const getTurboCost = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
 ...
 * @returns A promise that resolves with the velocity boost (as bigint)
 */
export const getTurboVelocity = async (
  level: number,
  provider?: ethers.Provider
): Promise<bigint> => {
  const contract = await getCosmicPartsContract(provider);
...
export const upgradePart = async (
  ...
  const signedTx = await contract.upgradePart.populateTransaction(partType, {
    gasLimit: 500000,
    nonce: nonce,
