...
class TransactionTracker {
  ...
  public setProvider(
    ...
    this.isWebSocketProvider =
      typeof (provider as ethers.WebSocketProvider).destroy === "function";

    console.log(
      `TransactionTracker using ${this.isWebSocketProvider ? "WebSocket" : "HTTP"} provider.`
    );
  ...
  public updatePendingTransaction(
    ...
    if (!existingRecord || existingRecord.status !== "pending") {
      console.warn(
        `Optimistic transaction ${placeholderHash} not found or not pending.`
      );
      return;
    }
    ...
    let isAsyncContractTransaction = false;
    ...
    if (typeof txOrHash === "string") {
      ...
      if (
        "transactionHash" in txOrHash &&
        typeof txOrHash.transactionHash === "string"
      ) {
        realHash = txOrHash.transactionHash;
        // Check if it's a contract transaction response with a wait method
        isContractTransaction =
          "wait" in txOrHash && typeof txOrHash.wait === "function";
        isAsyncContractTransaction = "l1BaseFeeScalar" in txOrHash;
      } else {
        // Try to find a hash property or a transaction hash in the object
        console.error(
          "Object provided to updatePendingTransaction has no valid hash property",
          txOrHash
        );
      ...
      console.error(
        "Invalid transaction type provided to updatePendingTransaction:",
        typeof txOrHash,
    ...
    if (isContractTransaction) {
      try {
        ...
        console.error(
          "Error calling tx.wait() in updatePendingTransaction:",
          err
        );
      }
    }
    ...
    if (isAsyncContractTransaction) {
      this.updateTransactionStatus(
        realHash,
        "mined",
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        txRecord.receipt || undefined
      );
      //   if (callbacks && callbacks.onMined && txRecord.receipt) {
      //     // eslint-disable-next-line @typescript-eslint/no-explicit-any
      //     callbacks.onMined(txRecord.receipt);
      //   }
    }
  }
  ...
  public removePendingTransaction(placeholderHash: string): void {
    if (this.transactions[placeholderHash]) {
      ...
      console.warn(
        `Attempted to remove non-existent pending transaction: ${placeholderHash}`
      );
    }
  }
  ...
  private removeTransaction(hash: string): void {
    if (this.transactions[hash]) {
      ...
      console.log(
        `Transaction ${hash} automatically removed after completion.`
      );
    }
  }
  ...
  private updateTransactionStatus(
    ...
    if (!tx || tx.status === "pending") {
      console.log(
        `Skipping update for ${hash}: current status ${tx?.status}, new status ${status}`
      );
      return;
    }
  ...
  private async checkPendingTransactions(): Promise<void> {
    ...
    console.log(
      "🚀 | TransactionTracker | checkPendingTransactions | this.provider:",
      this.provider
    );
      ...
      try {
        ...
        if (receipt) {
          ...
          if (receipt.status === 0) {
            ...
            console.log(
              `Transaction ${tx.hash} mined in block #${receipt.blockNumber}`
            );
          }
        } else {
  ...
  private loadFromStorage(): void {
    if (typeof window !== "undefined") {
      try {
        ...
        if (storedData) {
          const parsedData = JSON.parse(storedData) as Omit<
            TransactionRecord,
            "receipt"
          >[];
...
export const getTransactionTracker = (
  ...
  if (wsUrl && typeof window !== "undefined") {
    if (
      ...
      (webSocketProviderInstance.websocket as WebSocket).url !== wsUrl
    ) {
      try {
  ...
  if (!instance) {
    ...
    instance = new TransactionTracker(
      newWebSocketProvider ?? undefined,
      pollingInterval
    );
    ...
    console.log(
      "Updated existing TransactionTracker instance with new WebSocketProvider."
    );
  } else if (
    webSocketProviderInstance &&
    !webSocketProviderChanged &&
    ...
    console.log(
      "Set existing WebSocketProvider on TransactionTracker instance that lacked one."
    );
  }
...
export const updatePendingTransaction = (
  ...
  getTransactionTracker().updatePendingTransaction(
    placeholderHash,
    txOrHash,
    callbacks
  );
